...
bool LDPC_Coding_d::CUDA_Configuration(){
	/*create a grid containing 256 blocks with 256 threads of each block*/
	dim3 grid(DEFAULT_CUDA_BLOCK_NUM,1,1);
	dim3 threads(DEFAULT_CUDA_THREAD_NUM,1,1);
	/*local assignment to the data array*/
	int entity_d_data[4];
	entity_d_data[0] = CodeWord_Size;
	entity_d_data[1] = Info_Size;
	entity_d_data[2] = d_pitch_r;
	entity_d_data[3] = d_pitch_c;
	/*memory copy from host terminal to device terminal*/
	ret = cudaMemcpyToSymbol(CUDA_PARA_SET, entity_d_data, sizeof(int)*4, size_t(0), cudaMemcpyHostToDevice);
	checkCudaErrors(ret);
	if(ret != cudaSuccess){
		//printf("Error allocating memory!\n");
		return FAIL;
		...
		//printf("The constant value in GPU is: %d, %d, %d, %d\n", CUDA_PARA_SET[0], CUDA_PARA_SET[1], CUDA_PARA_SET[2], CUDA_PARA_SET[3]);
		printf("Success transfer constant values to device\n");
		return SUCCESS;
	}
...
/********************************************the accessing method is name_of_array[pitch*num_of_row*pitch/4+num_of_col]*******************************************************************************/
	...
	*/
	...
	int count, outer_count, inner_count, tmp;
	float product;

	//use shared memory to calculate the product of multiplication
	/*
	__shared__ float product[DEFAULT_CODEWORD_SIZE];   //static allocate shared memory space(64800*32B) wrong!!!!!!
	__shared__ float product2[DEFAULT_CODEWORD_SIZE];*/
	...
	if(tid_in_grid < entity_d.Info_Size){
		...
		//while (Index_Row_Matrix_d[tid_in_grid][count] != -1) {	
			...
			//while (Index_Row_Matrix_d[tid_in_grid][outer_count] != -1) {	
				inner_count = 0;
				tmp = entity_d.Index_Row_Matrix_d[entity_d.d_pitch_r*tid_in_grid/4+outer_count]; 
				//tmp = Index_Row_Matrix_d[tid_in_grid][outer_count];                      							 //tmp stores the index to search column
				while (entity_d.Index_Col_Matrix_d[entity_d.d_pitch_c*inner_count/4+tmp] != tid_in_grid) {			 //compare whether two coordinates equal to each other
					//while (Index_Col_Matrix_d[inner_count][tmp] != tid_in_grid) {	
					inner_count++;
				}
			...
			/*can also calculate r1 based on r0, needs access r0 memory*/
			//entity_d->r1_d[d_pitch_r*tid_in_grid/4+count] = 1.0-entity_d->r0_d[d_pitch_r*tid_in_grid/4+count];
			
			//r0_d[tid_in_grid][count] = (1.0 + product[tid_in_grid]) *0.5;
			//r1_d[tid_in_grid][count] = 1.0-r0_d[tid_in_grid][count];
...
__global__ void LDPC_Decoding_P2(LDPC_Coding_d entity_d){
	...
	if(tid_in_grid < entity_d.CodeWord_Size){
		...
		count = 0;
		__syncthreads();
		while (entity_d.Index_Col_Matrix_d[entity_d.d_pitch_c*count/4+tid_in_grid] != -1) {												//update the p0 p1
		//while (Index_Col_Matrix_d[count][tid_in_grid] != -1) {
			tmp = entity_d.Index_Col_Matrix_d[entity_d.d_pitch_c*count/4+tid_in_grid];
			//tmp = Index_Col_Matrix_d[count][tid_in_grid];
			inner_count = 0;
			while (entity_d.Index_Row_Matrix_d[entity_d.d_pitch_r*tmp/4+inner_count] != tid_in_grid) {
			//while (Index_Row_Matrix_d[tmp][inner_count] != tid_in_grid) {
				inner_count++;
			}
			product *= entity_d.r0_d[entity_d.d_pitch_r*tmp/4+inner_count];
			product2 *= entity_d.r1_d[entity_d.d_pitch_r*tmp/4+inner_count];
			//product[tid_in_grid] *= r0_d[tmp][inner_count];
			//product2[tid_in_grid] *= r1_d[tmp][inner_count];
			__syncthreads();
			count++;
		}
		...
		/*
		if(tid_in_grid < 10){
			printf("tid_in_grid: %d, p0_d[%d]: %.6f, p1_d[%d]: %.6f\n", tid_in_grid, tid_in_grid, entity_d.p0_d[tid_in_grid], tid_in_grid, entity_d.p1_d[tid_in_grid]);
		}*/
		__syncthreads();
		...
		//while (Index_Col_Matrix[count][tid_in_grid] != -1) {
			tmp = entity_d.Index_Col_Matrix_d[entity_d.d_pitch_c*count/4+tid_in_grid];
			//tmp = Index_Col_Matrix[count][tid_in_grid];
			inner_count = 0;
			while (entity_d.Index_Row_Matrix_d[entity_d.d_pitch_r*tmp/4+inner_count] != tid_in_grid) {
			//while (Index_Row_Matrix[tmp][inner_count] != tid_in_grid) {
				inner_count++;
			}
			...
			//q0_d[count][tid_in_grid] = (float)p0[tid_in_grid] / r0_d[tmp][inner_count];
			//q1_d[count][tid_in_grid] = (float)p1[tid_in_grid] / r1_d[tmp][inner_count];
			sum = entity_d.q0_d[entity_d.d_pitch_c*count/4+tid_in_grid] + entity_d.q1_d[entity_d.d_pitch_c*count/4+tid_in_grid];
			//sum = q0_d[count][tid_in_grid] + q1_d[count][tid_in_grid];
			__syncthreads();
			...
			//q0_d[count][tid_in_grid] /= (float)sum;
			//q1_d[count][tid_in_grid] /= (float)sum;
			__syncthreads();
			count++;
		}
