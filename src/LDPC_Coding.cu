...
__global__ void LDPC_Decoding_P2(LDPC_Coding_d entity_d){
	...
	__shared__ float p1_sh[DEFAULT_CUDA_THREAD_NUM*2];
	__shared__ float p0_init_sh[DEFAULT_CUDA_THREAD_NUM*2];
	...
	if(tid_in_grid < entity_d.CodeWord_Size){
		...
		count = 0;
		/*test*/
		p0_sh[threadIdx.x] = entity_d.p0_d[tid_in_grid];
		p1_sh[threadIdx.x] = entity_d.p1_d[tid_in_grid];
		p0_init_sh[threadIdx.x] = entity_d.p0_init_d[tid_in_grid];
		p1_init_sh[threadIdx.x] = entity_d.p1_init_d[tid_in_grid];
		
		for(int i = 0;i<entity_d.Col_Weight;i++){
			tmp_col_index_matrix[entity_d.Col_Weight*threadIdx.x+i] = entity_d.Index_Col_Matrix_d[entity_d.d_pitch_c*i/4+tid_in_grid];
		}
		...
		entity_d.de_info_seq_d[tid_in_grid] = (p1_sh[threadIdx.x] > 0.5) ? LDPC_1 : LDPC_0;
		
		entity_d.p0_d[tid_in_grid] = p0_sh[threadIdx.x];
		entity_d.p1_d[tid_in_grid] = p1_sh[threadIdx.x];
	}
